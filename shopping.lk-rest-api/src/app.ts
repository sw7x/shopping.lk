...
import globalErrorHandler from '@root/src/http/middlewares/errorHandlers/globalErrorHandler';
...
import createRateLimiterMiddleware from '@src/http/middlewares/rateLimiterMiddleware';
import pageNotFoundErrorHandler from '@root/src/http/middlewares/errorHandlers/pageNotFoundErrorHandler';
...
import successRespLogger from '@root/src/http/middlewares/responseLogger';
import xssProtectionMiddleware from '@src/http/middlewares/xssProtection';
import logger from '@src/setup/logger';

import httpError from '@src/shared/http/httpError';
import HttpStatusError from '@src/shared/errors/HttpStatusError';
...
//console.log('ADMIN_PASSWORD:' + config.get('DEFAULT_ADMIN_PASSWORD'));
...
//const uploadDir = path.join(__dirname, 'uploads'); // Directory to save uploaded files
...
app.get('/debug-info', (req: Request, res: Response, next: NextFunction) => {
	...
	try {
		...
		/*res.json({
			'debug-info': config.get('debug') + ' type of debug ' + typeof config.get('debug'),
			devx: 'susa',
			'vvv-info': config.get('vvv') + ' type of vvv ' + typeof config.get('vvv'),
			'zzz1-info': config.get('zzz1') + ' type of zzz1 ' + typeof config.get('zzz1'),
			'c222-info': config.get('c222') + ' type of c222 ' + typeof config.get('c222'),
		});*/

		res.send('susa7: ' + config.get('susa7'));
		//res.send('susa7: ' + process.env.susa7);
...
app.get('/sync-error', (req: Request, res: Response, next: NextFunction) => {
	...
	/*try {
		...
		// OR
		//throw new HttpStatusError(402, 'Payment required');
	} catch (error: unknown) {
...
//throw new Error('ddddddSync error - Something went wrong!');
